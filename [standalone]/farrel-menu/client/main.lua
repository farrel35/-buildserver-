...
local function SetupVehicleMenu()
    ...
    if Vehicle ~= 0 then
        ...
        if IsPedInAnyVehicle(ped) then
            ...
            local seatTable = {
                [1] = _U("options.driver_seat"),
                [2] = _U("options.passenger_seat"),
                [3] = _U("options.rear_left_seat"),
                [4] = _U("options.rear_right_seat"),
            }
            ...
            for i = 1, AmountOfSeats do
                ...
                VehicleMenu.items[seatIndex].items[newIndex] = {
                    id = i - 2,
                    title = seatTable[i] or _U("options.other_seats"),
                    icon = 'caret-up',
                    type = 'client',
...
RegisterCommand('radialmenu', function()
    if IsPoliceOrEMS() and not IsPauseMenuActive() and not inRadialMenu then
        setRadialState(true, true)
        SetCursorLocation(0.5, 0.5)
    end
end)

RegisterKeyMapping('radialmenu', _U("general.command_description"), 'keyboard', Config.Keybind)

-- Events

RegisterNetEvent('qb-radialmenu:client:noPlayers', function()
    ESX.ShowNotification(_U("error.no_people_nearby"), 'error', 2500)
end)
...
RegisterNetEvent('qb-radialmenu:client:openDoor', function(data)
    ...
    if closestVehicle ~= 0 then
        ...
        ESX.ShowNotification(_U("error.no_vehicle_found"), 'error', 2500)
    end
end)
        ...
        if GetPedInVehicleSeat(veh, -1) == ped then
            ...
            if DoesExtraExist(veh, extra) then
                if IsVehicleExtraTurnedOn(veh, extra) then
                    ...
                    ESX.ShowNotification(_U("error.extra_deactivated", {extra = extra}), 'error', 2500)
                else
                    SetVehicleExtra(veh, extra, 0)
                    ESX.ShowNotification(_U("success.extra_activated", {extra = extra}), 'success', 2500)
                end
            else
                ESX.ShowNotification(_U("error.extra_not_present", {extra = extra}), 'error', 2500)
            end
        else
            ESX.ShowNotification(_U("error.not_driver"), 'error', 2500)
        end
    end
end)
    ...
    if not HasHarnass then
        ...
        if IsSeatFree then
            if kmh <= 100.0 then
                ...
                ESX.ShowNotification(_U("info.switched_seats", {seat = data.title}))
            else
                ESX.ShowNotification(_U("error.vehicle_driving_fast"), 'error')
            end
        else
            ESX.ShowNotification(_U("error.seat_occupied"), 'error')
        end
    else
        ESX.ShowNotification(_U("error.race_harness_on"), 'error')
    end
end)
