...
local function setTime(hour, minute)
    ...
    -- print(_U('change', {value = argh, value2 = argm}))
    TriggerEvent('qb-weathersync:server:RequestStateSync')
    return true
end
...
RegisterNetEvent('qb-weathersync:server:setWeather', function(weather)
    ...
    if isAllowedToChange(src) then
        ...
        if src > 0 then
            if (success) then TriggerClientEvent('esx:showNotification', src, _U('updated'))
            else TriggerClientEvent('esx:showNotification', src, _U('invalid'))
            end
        end
    end
end)
...
RegisterNetEvent('qb-weathersync:server:setTime', function(hour, minute)
    ...
    if isAllowedToChange(src) then
        ...
        if src > 0 then
            if (success) then TriggerClientEvent('esx:showNotification', src, _U('change', {value = hour, value2 = minute or "00"}))
            else TriggerClientEvent('esx:showNotification', src, _U('invalid'))
            end
        end
    end
...
RegisterNetEvent('qb-weathersync:server:toggleBlackout', function(state)
    ...
    if isAllowedToChange(src) then
        ...
        if src > 0 then
            if (newstate) then TriggerClientEvent('esx:showNotification', src, _U('enabled'))
            else TriggerClientEvent('esx:showNotification', src, _U('disabled'))
            end
        end
    end
end)
...
RegisterNetEvent('qb-weathersync:server:toggleFreezeTime', function(state)
    ...
    if isAllowedToChange(src) then
        ...
        if src > 0 then
            if (newstate) then TriggerClientEvent('esx:showNotification', src, _U('now_frozen'))
            else TriggerClientEvent('esx:showNotification', src, _U('now_unfrozen'))
            end
        end
    end
end)
...
RegisterNetEvent('qb-weathersync:server:toggleDynamicWeather', function(state)
    ...
    if isAllowedToChange(src) then
        ...
        if src > 0 then
            if (newstate) then TriggerClientEvent('esx:showNotification', src, _U('now_unfrozen'))
            else TriggerClientEvent('esx:showNotification', src, _U('now_frozen'))
            end
        end
    end
end)
...
RegisterCommand('freezetime', function(source, args)
    ...
    if source > 0 then
        if (newstate) then return TriggerClientEvent('esx:showNotification', source, _U('frozenc')) end
        return TriggerClientEvent('esx:showNotification', source, _U('unfrozenc'))
    end
    if (newstate) then return print(_U('now_frozen')) end
    return print(_U('now_unfrozen'))
end)

RegisterCommand('freezeweather', function(source, args)
    local newstate = setDynamicWeather()
    if source > 0 then
        if (newstate) then return TriggerClientEvent('esx:showNotification', source, _U('dynamic_enabled')) end
        return TriggerClientEvent('esx:showNotification', source, _U('dynamic_disabled'))
    end
    if (newstate) then return print(_U('now_unfrozen')) end
    return print(_U('now_frozen'))
end)

RegisterCommand('weather', function(source, args)
    local success = setWeather(args[1])
    if source > 0 then
        if (success) then return TriggerClientEvent('esx:showNotification', source, _U('willchangeto', args[1])) end
        return TriggerClientEvent('esx:showNotification', source, _U('invalidc'), 'error')
    end
    if (success) then return print(_U('updated')) end
    return print(_U('invalid'))
end, false)

RegisterCommand('blackout', function(source, args)
    local newstate = setBlackout()
    if source > 0 then
        if (newstate) then return TriggerClientEvent('esx:showNotification', source, _U('enabledc')) end
        return TriggerClientEvent('esx:showNotification', source, _U('disabledc'))
    end
    if (newstate) then return print(_U('enabled')) end
    return print(_U('disabled'))
end, false)

RegisterCommand('time', function(source, args)
    local success = setTime(args[1], args[2])
    if source > 0 then
        if (success) then return TriggerClientEvent('esx:showNotification', source, _U('change', args[1], args[2])) end
        return TriggerClientEvent('esx:showNotification', source, _U('invalidc'), 'error')
    end
    if (success) then return print(_U('change', args[1],args[2])) end
    return print(_U('invalid'))
end, false)
