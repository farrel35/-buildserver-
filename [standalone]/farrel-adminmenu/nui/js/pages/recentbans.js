...
FARREL.AdminMenu.BanList.SelectedType = "all";
FARREL.AdminMenu.BanList.CreatedBanTypes = false;

FARREL.AdminMenu.LoadBanList = function() {
    DebugMessage('Loading Bans.')

    if (FARREL.AdminMenu.Bans != "" && FARREL.AdminMenu.Bans != null) {
        $('.menu-page-recentbans-types').html('');
        $('.admin-menu-bans').html('');
        $('.no-bans').hide();
        FARREL.AdminMenu.BuildTypes();
        FARREL.AdminMenu.LoadFilteredBanList(FARREL.AdminMenu.BanList.SelectedType);
    } else {
        $('.menu-page-recentbans-types').html('');
        $('.admin-menu-bans').html('');
        $('.no-bans').fadeIn(450);
        DebugMessage('No bans found.');
    }   
}

FARREL.AdminMenu.BuildTypes = function() {
    $('.menu-page-recentbans-types').html('');
    ...
    // Add Other
    for (let i = 0; i < FARREL.AdminMenu.BanTypes.length; i++) {
        let Category = FARREL.AdminMenu.BanTypes[i];
...
FARREL.AdminMenu.LoadFilteredBanList = function(Type) {
    $('.admin-menu-bans').html('');
    FARREL.AdminMenu.BanList.SelectedType = Type;
    FARREL.AdminMenu.BanList.CreatedBanTypes = true;
    $.post(`https://${GetParentResourceName()}/Admin/GetDateDifference`, JSON.stringify({ 
        CType: Type,
        BanList: FARREL.AdminMenu.Bans,
    }), function(Data) {
        if (Data.Bans) {
            for (let i = 0; i < Data.Bans.length; i++) {
                let BannedPlayer = Data.Bans[i];
                FARREL.AdminMenu.BuildFilteredBanList(BannedPlayer, i);
                console.log("test")
            }
        }
    });
...
FARREL.AdminMenu.BuildFilteredBanList = function(BannedPlayer, PlayerId) {
    ...
    let BannedPElem = `<div class="admin-menu-ban" id="player-ban-${PlayerId}">
                ...
                <div class="admin-menu-ban-options">
                    <div class="admin-menu-ban-option">
                        ...
                        <div class="admin-menu-ban-option-desc" id='recentbans-steam'>
                            <div class="admin-menu-ban-option-copy"><i class="fa-solid fa-copy"></i></div>
                    ...
                    <div class="admin-menu-unban ui-styles-button error">Unban</div>
                </div>
            </div>`;
...
$(document).on('click', '.admin-menu-ban-option #recentbans-discord', function(e) {
    ...
    FARREL.AdminMenu.Copy(Discord);
    FARREL.AdminMenu.ShowCheckmark();
});

$(document).on('click', '.admin-menu-ban-option #recentbans-steam', function(e) {
    e.preventDefault();
    let License = $(this).find('p').text();
    FARREL.AdminMenu.Copy(License);
    FARREL.AdminMenu.ShowCheckmark();
});
...
$(document).on('click', '.menu-page-recentbans-types', function(e) {
    ...
    FARREL.AdminMenu.LoadFilteredBanList(Type)
});

$(document).on('click', '.admin-menu-unban', function(e) {
    let Data = $(this).parent().parent().data('BanData');
    $.post(`https://${GetParentResourceName()}/Admin/UnbanPlayer`, JSON.stringify({ 
        PData: Data, 
    }));
    FARREL.AdminMenu.ShowCheckmark();
    setTimeout(() => {
...
$(document).on('input', '#banslist-steam', function(e){
    let SearchText = $(this).val().toLowerCase();

    $('.admin-menu-ban').each(function(Elem, Obj){
        if ($(this).find('#recentbans-steam').find('p').html().toLowerCase().includes(SearchText)) {
            $(this).fadeIn(150);
        } else {
            $(this).fadeOut(150);
        };
