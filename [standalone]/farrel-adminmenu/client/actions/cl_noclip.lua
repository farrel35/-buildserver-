...
function toggleFreecam(enabled)
    ...
    if not IsFreecamActive() and enabled then
        ESX.ShowNotification('Noclip '.._U("enabled"), 'success')
        enableNoClip()
    end

    if IsFreecamActive() and not enabled then
        ESX.ShowNotification('Noclip '.._U("disabled"), 'error')
        disableNoClip()
    end
end
...
function StartFreecamThread()
  ...
  CreateThread(function()
    ...
    InstructionalButton(GetControlInstructionalButton(0, 21, 1), _U('faster')) -- Left Shift
    PopScaleformMovieFunctionVoid()
      
    PushScaleformMovieFunction(scaleform, "SET_DATA_SLOT")
    PushScaleformMovieFunctionParameterInt(1)
    InstructionalButton(GetControlInstructionalButton(0, 19, 1), _U('slower')) -- left Alt
    PopScaleformMovieFunctionVoid()
      
    PushScaleformMovieFunction(scaleform, "SET_DATA_SLOT")
    PushScaleformMovieFunctionParameterInt(2)
    InstructionalButton(GetControlInstructionalButton(0, 31, 1), _U('fwdback')) -- S
    PopScaleformMovieFunctionVoid()
      
    PushScaleformMovieFunction(scaleform, "SET_DATA_SLOT")
    PushScaleformMovieFunctionParameterInt(3)
    InstructionalButton(GetControlInstructionalButton(0, 30, 1), _U('leftright')) -- D
    PopScaleformMovieFunctionVoid()
  
    PushScaleformMovieFunction(scaleform, "SET_DATA_SLOT")
    PushScaleformMovieFunctionParameterInt(4)
    InstructionalButton(GetControlInstructionalButton(0, 153, 1), _U('down')) -- E
    PopScaleformMovieFunctionVoid()
  
    PushScaleformMovieFunction(scaleform, "SET_DATA_SLOT")
    PushScaleformMovieFunctionParameterInt(5)
    InstructionalButton(GetControlInstructionalButton(0, 152, 1), _U('up')) -- Q
    PopScaleformMovieFunctionVoid()
