...
local GetEntityInfo = function(entity)
    ...
    local entityName    = Entities[entityHash] or _U("obj_unknown")
    local entityData    = {
        '~y~'.._U("entity_view_info"),
        '',
        _U("model_hash")..' ~y~'..entityHash,
        ' ',
        _U("ent_id")..' ~y~'..entity,
        _U("obj_name")..' ~y~'.. entityName,
        _U("net_id")..' ~y~'..(NetworkGetEntityIsNetworked(entity) and NetworkGetNetworkIdFromEntity(entity) or _U("net_id_not_registered")),
        _U("ent_owner")..' ~y~'..GetPlayerServerId(NetworkGetEntityOwner(entity)),
        ' '
    }

    if entityType == 1 then
        local pedRelationshipGroup = GetPedRelationshipGroupHash(entity)
        table.insert(entityData, _U("cur_health")..' ~y~'..GetEntityHealth(entity))
        table.insert(entityData, _U("max_health")..' ~y~'..GetPedMaxHealth(entity))
        table.insert(entityData, _U("armour")..' ~y~'..GetPedArmour(entity))
        table.insert(entityData, _U("rel_group")..' ~y~'.. (Entities[pedRelationshipGroup] or _U("rel_group_custom")))
        table.insert(entityData, _U("rel_to_player")..' ~y~'..GetPedRelationshipType(GetRelationshipBetweenPeds(pedRelationshipGroup, PlayerPedId())))
    elseif entityType == 2 then
        table.insert(entityData, _U("veh_rpm")..' ~y~'..RoundFloat(GetVehicleCurrentRpm(entity), 2))
        table.insert(entityData, (useKph and _U("veh_speed_kph") or _U("veh_speed_mph"))..' ~y~'..RoundFloat((GetEntitySpeed(entity)*(useKph and 3.6 or 2.23694)), 0))
        table.insert(entityData, _U("veh_cur_gear")..' ~y~'..GetVehicleCurrentGear(entity))
        table.insert(entityData, _U("veh_acceleration")..' ~y~'..RoundFloat(GetVehicleAcceleration(entity), 2))
        table.insert(entityData, _U("body_health")..' ~y~'..GetVehicleBodyHealth(entity))
        table.insert(entityData, _U("eng_health")..' ~y~'..GetVehicleEngineHealth(entity))
    ...
    table.insert(entityData, _U("dist_to_obj")..' ~y~'.. RoundFloat(#(playerCoords-entityCoords), 2))
    table.insert(entityData, _U("obj_heading")..' ~y~'.. RoundFloat(GetEntityHeading(entity), 2))
    table.insert(entityData, _U("obj_coords")..' ~y~'.. RoundVector3(entityCoords, 2))
    table.insert(entityData, _U("obj_rot")..' ~y~'.. RoundVector3(GetEntityRotation(entity), 2))
    table.insert(entityData, _U("obj_velocity")..' ~y~'.. RoundVector3(GetEntityVelocity(entity), 2))
    ...
    CreateThread(function()
        while EntityViewEnabled do
            ...
            if EntityFreeAim then
                DrawTitle("~y~".._U("entity_view_title").."~w~\n\n[~r~E~w~] - ".._U("entity_freeaim_delete").."~w~\n[~b~G~w~] - ".._U("entity_freeaim_freeze").."~w~\n[~g~H~w~]Copy", 0.15, 0.17)
                ...
                if hit and (IsEntityAVehicle(entity) or IsEntityAPed(entity) or IsEntityAnObject(entity)) then
                    ...
                    if IsControlJustReleased(0, 47) then -- Freeze entities
                        ...
                        ESX.ShowNotification(_U("you_have")..(FrozenEntities[entity] and _U("entity_frozen") or _U("entity_unfrozen")).. _U("freeaim_entity"), (FrozenEntities[entity] and 'success' or 'error'))
                    end
                    ...
                    if IsControlJustReleased(0, 38) then -- Delete entity
                        ...
                        if not DoesEntityExist(entity) then
                            ESX.ShowNotification(_U("entity_del"), 'success')
                        else
                            ESX.ShowNotification(_U("entity_del_error"), 'error')
                        end
                    end
